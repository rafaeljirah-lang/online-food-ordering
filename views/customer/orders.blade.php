...
<div class="container mx-auto px-4 py-12">
    ...
    <div class="bg-white border-2 border-gray-200 rounded-lg p-6 mb-4 hover:border-primary transition">
        <div class="flex justify-between items-start mb-4">
            <div>
                <h3 class="text-xl font-bold text-dark">Order #{{ $order->id }}</h3>
        ...
        <div class="mb-4 space-y-2">
            ...
            <div class="flex items-center gap-3">
                ...
                @endif
                <div class="flex-1">
                    <p class="font-semibold text-dark">{{ $item->menuItem->name }}</p>
            ...
            <p class="text-sm text-gray-600 ml-15">+ {{ $order->items->count() - 3 }} more items</p>
            @endif
        </div>
        ...
        <div class="flex justify-between items-center pt-4 border-t-2 border-gray-200">
            ...
            <div class="flex gap-3">
                <a href="{{ route('customer.order-details', $order->id) }}" class="bg-primary text-white px-6 py-2 rounded-lg hover:opacity-90 transition">
                    View Details
                </a>
                @if($order->status == 'completed')
                <button class="bg-dark text-white px-6 py-2 rounded-lg hover:opacity-90 transition">
                    Reorder
                </button>
                @endif
            </div>
