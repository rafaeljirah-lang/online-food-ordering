    ...
    <div class="max-w-2xl w-full">
        ...
        <div class="bg-white border-2 border-gray-200 rounded-lg p-8 mb-6">
            <div class="text-center mb-6">
                ...
                <p class="text-4xl font-bold text-primary">#{{ $order->id ?? '' }}</p>
            </div>

            <div class="border-t-2 border-gray-200 pt-6 space-y-4">
                <div class="flex justify-between">
                    <span class="text-gray-600">Order Date:</span>
                    <span class="font-semibold text-dark">{{ $order->created_at->format('M d, Y - h:i A') }}</span>
                </div>
                <div class="flex justify-between">
                    <span class="text-gray-600">Total Amount:</span>
                    <span class="font-bold text-2xl text-primary">${{ number_format($order->total_amount, 2) }}</span>
                </div>
                <div class="flex justify-between">
                    <span class="text-gray-600">Payment Method:</span>
                    <span class="font-semibold text-dark">{{ ucfirst($order->payment_method) }}</span>
                </div>
                <div class="flex justify-between">
                    <span class="text-gray-600">Delivery Address:</span>
                    <span class="font-semibold text-dark text-right">{{ $order->delivery_address }}</span>
                </div>
            </div>
        </div>
        ...
        <div class="grid md:grid-cols-2 gap-4">
            <a href="{{ route('customer.order-details', $order->id) }}" class="bg-primary text-white px-6 py-3 rounded-lg font-semibold text-center hover:opacity-90 transition">
                View Order Details
            </a>
            <a href="{{ route('customer.menu') }}" class="bg-dark text-white px-6 py-3 rounded-lg font-semibold text-center hover:opacity-90 transition">
                Continue Shopping
            </a>
        </div>
        ...
        <div class="text-center mt-6">
            <a href="{{ route('customer.home') }}" class="text-primary hover:underline">
                ← Back to Home
            </a>
        </div>
    </div>
